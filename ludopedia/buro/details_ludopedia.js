...
async function scrapeProductDetails(url, browser) {
  ...
  try {
    ...
    // Save images
    const imagesDir = path.join(__dirname, "buro");
    ...
    return {
      ...
      isVisible: true,
      editors: ["buro"], // Assuming this is fixed
...
async function main() {
  const links = JSON.parse(fs.readFileSync("../links/links_buro_brasil.json"));
  ...
  console.log("Detailed products data saved to detailed_buro_brasil.json");
